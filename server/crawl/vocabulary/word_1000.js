...
const fs = require('fs');
module.exports = class Word1000 {
    constructor() {
        let self = this;
        this.url = 'https://www.tienganh123.com/1000-tu-vung-tieng-anh-co-ban';
    ...
    getLessonDetail(url, lessonIndex) {
        return this.loader.load(url).then($ => {
            let exampleSentenceVIUrl = `https://noidung.tienganh123.com/file/baihoc/vocabulary/1000/bai${lessonIndex}/bai_${lessonIndex}.js`;
    ...
    getData() {
        ...
        return this.getAllLesson().then(lesson => {
            fs.writeFileSync(
                'static/data/lesson_1000_vocabulary.json',
            ...
            lesson.forEach(item => {
                lessonIndexData.lesson.push({
                    ...
                    dataUrl: `data/word_1000/lesson_${item.id}.json`
                });
                fs.writeFileSync(
                    `static/data/word_1000/lesson_${item.id}.json`,
                    JSON.stringify(item).replace(/\s+/g, ' ').replace(/-\s/g, '')
                );
            });
            fs.writeFileSync(
                `static/data/word_1000/index.json`,
